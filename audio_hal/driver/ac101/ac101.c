...
#define AC_ASSERT(a, format, b, ...) \
    if ((a) != 0) { \
        ESP_LOGE(AC_TAG, format, ##__VA_ARGS__); \
        return b;\
    }
...
esp_err_t AC101_init(audio_hal_codec_config_t* codec_cfg) {
	...
	if (ESP_OK != res) {
		ESP_LOGE(AC_TAG, "reset failed!");
		return res;
	} else {
		ESP_LOGI(AC_TAG, "reset succeed");
	}
...
esp_err_t AC101_start(ac_module_t mode)
{
    ...
    if (mode == AC_MODULE_LINE) {
		...
		res |= AC101_Write_Reg(ADC_DIG_CTRL, 0x800e);
...
esp_err_t AC101_ctrl_state(audio_hal_codec_mode_t mode, audio_hal_ctrl_t ctrl_state)
{
	...
	switch (mode) {
		...
		default:
			es_mode_t = AC_MODULE_DAC;
			ESP_LOGW(AC_TAG, "Codec mode not support, default is decode mode");
			break;
	}
...
esp_err_t AC101_set_voice_mute(bool enable)
{
	...
	if (enable) {
		...
		res |= AC101_Write_Reg(HPOUT_CTRL, 0xfbc0); 	//enable earphone
		res |= AC101_Write_Reg(SPKOUT_CTRL, 0xeabd); 	//enable speaker
	}
    return res;
}
